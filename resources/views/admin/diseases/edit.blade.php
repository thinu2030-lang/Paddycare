...
<style>
    ...
    .form-control { width: 100%; padding: 10px 14px; border: 1.5px solid #d0ddd4; border-radius: 8px; font-size: 13px; background: #f8f9fa; transition: border-color 0.2s; }
...
<div class="dashboard-wrap">
    ...
    <main class="main-content">
        <div class="page-title">✏️ Edit Disease — {{ $disease->name }}</div>
        <div class="form-card">
            <form method="POST" action="{{ route('admin.diseases.update', $disease->id) }}">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="scientific_name" class="form-control" value="{{ old('scientific_name',$disease->scientific_name) }}">
                    </div>
                </div>
                <div class="form-group">
                    <label class="form-label">Severity Level</label>
                    <select name="severity_level" class="form-control">
                        <option value="high"   {{ old('severity_level',$disease->severity_level)==='high'  ?'selected':'' }}>🔴 High</option>
                        <option value="medium" {{ old('severity_level',$disease->severity_level)==='medium'?'selected':'' }}>🟡 Medium</option>
                        <option value="low"    {{ old('severity_level',$disease->severity_level)==='low'   ?'selected':'' }}>🟢 Low</option>
                    </select>
                </div>
                <div class="form-group">
                    <label class="form-label">Description</label>
                    <textarea name="description" class="form-control" rows="3" required>{{ old('description',$disease->description) }}</textarea>
                </div>
                <div class="form-group">
                    <label class="form-label">🧪 Chemical Treatment</label>
                    <textarea name="chemical_treatment" class="form-control" rows="3" required>{{ old('chemical_treatment',$disease->chemical_treatment) }}</textarea>
                </div>
                <div class="form-group">
                    <label class="form-label">🌿 Organic Treatment</label>
                    <textarea name="organic_treatment" class="form-control" rows="3" required>{{ old('organic_treatment',$disease->organic_treatment) }}</textarea>
                </div>
                <div class="form-group">
                    <label class="form-label">♻️ IPM Advice</label>
                    <textarea name="ipm_advice" class="form-control" rows="3" required>{{ old('ipm_advice',$disease->ipm_advice) }}</textarea>
                </div>
                <div style="display:flex; gap:10px; margin-top:8px;">
                    <button type="submit" class="btn-submit">💾 Update Disease</button>
                    <a href="{{ route('admin.diseases.index') }}"><button type="button" class="btn-back">← Back</button></a>
                </div>
            </form>
        </div>
    </main>
