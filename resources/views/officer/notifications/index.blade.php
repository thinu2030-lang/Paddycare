...
<style>
    ...
    td { padding: 13px 16px; font-size: 13px; border-bottom: 1px solid #f0f4f1; }
    tr:last-child td { border-bottom: none; }
    tr:hover td { background: #f8fdf9; }
    .badge { font-size: 11px; padding: 4px 12px; border-radius: 20px; font-weight: 500; }
    .type-fertilizer    { background: #e8f5e9; color: #1a5c2e; }
    .type-meeting       { background: #e6f1fb; color: #185fa5; }
    .type-disease_alert { background: #fce4ec; color: #993556; }
    .type-general       { background: #fff3e0; color: #854f0b; }
...
<div class="dashboard-wrap">
    ...
    <main class="main-content">
        <div class="page-header">
            <div class="page-title">📧 Sent Notifications</div>
            <a href="{{ route('officer.notifications.create') }}">
                <button class="btn-primary">📨 Send New Notification</button>
            </a>
        </div>
        ...
        <div class="card">
                    ...
                    <tr>
                        <td>
                            <span class="badge type-{{ $n->type }}">
                                {{ $labels[$n->type] ?? ucfirst($n->type) }}
                            </span>
                        </td>
                        <td style="font-weight:500;">{{ $n->subject }}</td>
                        <td>📍 {{ $n->district }}</td>
                        <td>👨‍🌾 {{ $n->recipients_count }} farmers</td>
            ...
            <div class="empty-state">
                ...
                <a href="{{ route('officer.notifications.create') }}"
                   style="color:#1a5c2e; font-weight:600; margin-top:8px; display:inline-block;">
