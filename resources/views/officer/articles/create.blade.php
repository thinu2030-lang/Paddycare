...
<style>
    ...
    .image-upload-box {
        ...
        padding: 20px; text-align: center; background: #f8fdf9;
...
<div class="dashboard-wrap">
    ...
    <main class="main-content">
        <div class="page-title">📝 Write New Article</div>
        <div class="form-card">
            <form method="POST" action="{{ route('officer.articles.store') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title" class="form-control"
                           value="{{ old('title') }}"
                           placeholder="e.g. Best Practices for Pest Control" required
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select name="category" class="form-control" required>
                            <option value="irrigation"  {{ old('category')==='irrigation' ?'selected':'' }}>💧 Irrigation</option>
                            <option value="fertilizer"  {{ old('category')==='fertilizer' ?'selected':'' }}>🧪 Fertilizer</option>
                            <option value="weed"        {{ old('category')==='weed'       ?'selected':'' }}>✂️ Weed Control</option>
                            ...
                            <option value="seed"        {{ old('category')==='seed'       ?'selected':'' }}>🌱 Seed Selection</option>
                        </select>
                    </div>
                    <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <textarea name="content" class="form-control" rows="10"
                              placeholder="Write detailed article content here..." required>{{ old('content') }}</textarea>
                </div>
                <div style="display:flex; gap:10px;">
                    <button type="submit" class="btn-submit">📤 Publish Article</button>
                    <a href="{{ route('officer.articles.index') }}">
